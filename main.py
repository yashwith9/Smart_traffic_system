...
class SmartTrafficPipeline:
    ...
    def initialize(self) -> bool:
        try:
            ...
            print("Model not found. Train first with: python rl/train_rl.py")
            return False
    ...
    def run_mock(self, steps: int = 30, interval: float = 1.0) -> None:
        print("Running full pipeline in MOCK mode...")
        try:
    ...
    def run_camera(self, source: str = "0", show_window: bool = True) -> None:
        ...
        print("Running full pipeline in CAMERA mode. Press 'q' to quit.")
        try:
            while True:
                ok, frame = cap.read()
                if not ok:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model", type=str, default="rl/q_table.pkl", help="Path to Q-table model")
    ...
    parser.add_argument("--serial-port", type=str, default="COM5", help="ESP32/Arduino serial port")
    parser.add_argument("--serial-baud", type=int, default=115200, help="Serial baudrate")
